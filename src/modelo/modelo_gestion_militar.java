...
public class modelo_gestion_militar {
    ...
    public Soldado subirRango(Soldado soldado) {
        if (soldado instanceof SoldadoRaso) {
            SoldadoRaso.remove();
            Teniente tenienteNuevo = new Teniente(soldado.getNombre(), soldado.getId());
            addSoldado(tenienteNuevo);
            return tenienteNuevo;
        } else if (soldado instanceof Teniente) {
            Teniente.remove();
            Capitan capitanNuevo = new Capitan(soldado.getNombre(), soldado.getId());
            addSoldado(capitanNuevo);
            return capitanNuevo;
        } else if (soldado instanceof Capitan) {
            Capitan.remove();
            Coronel coronelNuevo = new Coronel(soldado.getNombre(), soldado.getId());
            return coronelNuevo;
        } else {
            return null;
        }
    ...
    public Soldado bajarRango(Soldado soldado) {
        if (soldado instanceof Coronel) {
            ...
            Teniente.remove();
            SoldadoRaso soldadoRasoNuevo = new SoldadoRaso(soldado.getNombre(), soldado.getId());
            addSoldado(soldadoRasoNuevo);
            return soldadoRasoNuevo;
        } else {
            SoldadoRaso.remove();
            return null;
        }
    ...
    public String mensaje_asignar_mision(Soldado soldado, String mision){
        String mensaje = "";
        if (soldado instanceof SoldadoRaso) {
            SoldadoRaso soldadonew = (SoldadoRaso) soldado;
